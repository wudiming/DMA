  ...
  const handleShowUpdate = async (containerSummary) => {
    try {
      ...
      const initialData = {
        ...
        entrypoint: container.Config?.Entrypoint ? container.Config.Entrypoint.join(' ') : '',
        cmd: container.Config?.Cmd ? container.Config.Cmd.join(' ') : '',
        capAdd: container.HostConfig?.CapAdd || [],
        devices: container.HostConfig?.Devices?.map(d => ({ PathOnHost: d.PathOnHost, PathInContainer: d.PathInContainer, CgroupPermissions: d.CgroupPermissions })) || [],
        sysctls: container.HostConfig?.Sysctls ? Object.entries(container.HostConfig.Sysctls).map(([key, value]) => ({ key, value })) : []
      };
